...
class Config:
    def __init__(self, type_config, width=20, height=20, snap_to_grid=False,
        ...
        @param type_config  json file or object mapping types to 0/1 matrices
            indicating type shapes
        @param width    number of vertical 'blocks' on the board, e.g. for
            block-based rendering. default:20
        @param height   number of horizontal 'blocks' on the board, e.g. for
            block-based rendering. default:20
        @param snap_to_grid     True to lock objects to the nearest block at
            gripper release. default:False
        @param prevent_overlap  True to prohibit any action that would lead to
            objects overlapping. default:True
        @param actions  array of strings naming allowed object manipulations.
            default:['move', 'rotate']
        @param move_step    step size for object movement. default:0.2[blocks]
        @param rotation_step    applied angle when object is rotated.
            Limitations might exist for View implementations. default:90
        @param action_interval  frequency of repeating looped actions in
            seconds. default: 0.5
        """
        self.width = width
        self.height = height
    ...
    def _types_from_JSON(self, filename):
        ...
        The file should map each supported object type to a grid filled with 0s
        and 1s, where a 1 signifies the presence of a block and a 0 signifies
        the absence.
