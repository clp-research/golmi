$(document).ready(function () {
     ...
     * @param {replay speed multiplier, default: 1} speed
     * @param {ScrollableDisplay instance to display info next to the replay,
     *         default: null} metaDataDisplay
     */
    this.Replayer = class Replayer {
        constructor(frameRate=20, speed=1, metaDataDisplay=null) {
            this._log;
            if (frameRate > 100) {
                console.log("Warning:",
        ...
        get endTime() {
            if (this._endTime != undefined) {
                if (this.log) {
                    return Math.min(this._endTime, 
                                    this.log[this.log.length-1][0]);
                }
        ...
        _startReadingLog() {
            ...
            this.replayLoop = setInterval(() => {
                if (this.currentTime < this.endTime) {
                    ...
                    this._fastForward(this, 
                        this.currentTime + frameOffset*parseFloat(this.speed),
                        true);
                } else {
                    this._stopReadingLog();
                }
        ...
        _makeUpdate(update, notify=true) {
            ...
            //TODO: timestamp?
            if (update["event"] && this.metaDataDisplay) {
                if (update["event"] == "startAction") {
                    let msg = "";
                    if (update["loop"]) {
                        msg += "<span style='color:green;'>START:</span> ";
                    }
                    this.metaDataDisplay.addMessage(msg + update["type"]);
                } else if (update["event"] == "stopAction") {
                    this.metaDataDisplay.addMessage(
                        "<span style='color:red;'>STOP:</span> " +
                        update["type"]);
                } else if (update["event"] == "emitMessage") {
                    console.log("Replayer._makeUpdate():",
                        "processing 'emitMessage' event not implemented");
                }
            }
        }
